          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3.5 py-1.5 text-xs font-bold rounded-full transition-all cursor-pointer ${
                activeCategory === cat
                  ? 'bg-sky-600 text-white font-black shadow-sm scale-102'
        ...
        >
          {filteredFleet.map((vehicle) => (
            <SwiperSlide key={vehicle.id} className="h-auto">
              <div className="glass-card-light rounded-2xl md:rounded-3xl overflow-hidden flex flex-col justify-between h-full border border-slate-200 hover:border-sky-400 group shadow-2xs">
                ...
                <div className="relative h-40 md:h-44 overflow-hidden bg-slate-100 shrink-0">
                  ...
                  <div className="absolute top-3 right-3 bg-sky-600 text-white font-black px-3 py-0.5 rounded-full text-[11px] shadow-sm">
                    {vehicle.rate}
                  </div>

                  {vehicle.popular && (
                    <div className="absolute top-3 left-3 bg-amber-500 text-white font-black px-2.5 py-0.5 rounded-full text-[9px] uppercase tracking-wider shadow-2xs">
                      POPULAR CHOICE
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 md:p-5 flex-1 flex flex-col justify-between space-y-3">
                  <div>
                    <span className="text-[9px] md:text-[10px] font-black text-amber-700 uppercase tracking-widest block">{vehicle.category}</span>
                    <h3 className="text-base md:text-lg font-black text-slate-900 font-heading mt-0.5 group-hover:text-sky-600 transition-colors leading-tight">
                      {vehicle.name}
                    </h3>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-[11px] bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-700 font-semibold truncate">
                      <Fuel className="w-3 h-3 text-blue-600 shrink-0" />
